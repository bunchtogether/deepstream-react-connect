...
export default class Marshaller {
  ...
  marshall = (element:Element<any>):Instance => {
    const children = [];
    React.Children.map(element.props.children, (child) => {
      if (React.isValidElement(child)) {
        children.push(this.marshall(child));
      } else {
        children.push(this.marshall(<Text value={child} />));
      }
    });
  ...
  unmarshall = (instance:Instance):Element<any> => {
    ...
    const children = instance.children.map((childInstance: Instance) => {
      ...
      return this.unmarshall(childInstance);
    });
